pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'UNRAR_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-unrar'
    CONTAINER_NAME='unrar'
    DOCKERHUB_IMAGE = 'linuxserver/unrar'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/unrar'
    PR_DOCKERHUB_IMAGE = 'lspipepr/unrar'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='false'
    CI_WEB='false'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/unrar") | .uuid' || :)
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/unrar",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-unrar",\
                       "backendUrl":"https://ghcr.io/linuxserver/unrar",\
                       "publicUrl":"https://lscr.io/linuxserver/unrar"}' || :
              else
